...
export function createMongooseSchemaProvider<TEntity, TModel extends Model<TEntity>>(
  token: Token<TModel>,
  schemaName: string,
  ...
  return factoryProvider(
    ...
    (mongooseService: MongooseService) => {
      ...
      return (
        (connection.models[schemaName] as TModel | undefined) ??
        connection.model<TEntity>(schemaName, schema)
      ) as TModel;
    },
    [MONGOOSE_SERVICE],
  );
